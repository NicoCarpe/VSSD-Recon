"""VSSD-Recon with the scaffold defects of `vssd_recon.py` corrected.

Identical to `vssd_recon.py` except for three changes, all outside the VSSD block
itself, so a comparison against it isolates the scaffold rather than the mixer:

1. `FinalProjection(norm_output=False)`. The original normalises the *output*
   channels as its last operation, which pins ||out(h,w)||_2 to sqrt(out_chans) at
   every pixel: the head can choose the direction of its correction but not the
   magnitude, at every one of the `num_cascades` steps. Here the LayerNorm moves to
   the features before the projection.

2. `dim_scale` and the padding multiple are tied to `patch_size`. The original
   hard-codes a 4x expansion and `pad_to = 32`, so `patch_size` is only correct at 4
   and the denoiser can never run finer than H/4. Other values do not raise: at
   patch_size=2 the denoiser returns a field twice the padded size in each dimension
   and `NormPromptUnet.unpad` slices the top-left corner out of it, so the model
   trains on a spatially misaligned correction with no error. Measured on a 64x64
   input: patch_size=2 -> denoiser emits 128x128, cropped to 64x64; patch_size=1 ->
   256x256, cropped to 64x64. With both tied to patch_size, 1/2/4 are all correct.

3. `ssd_positve_dA` defaults to True, matching upstream VSSD. The original pins it
   False, which leaves dA = dt*A negative (A = -exp(A_log)) so the aggregated term
   enters with the opposite sign to the D skip.

Set `ssd_positve_dA: false` in the config to keep change 3 out of the comparison.
"""
import math
import copy
...
class PromptUnet(nn.Module): 
    def __init__(self,
        ...
        # OutConv: expansion tied to patch_size, and no normalisation of the
        # output channels (see module docstring, change 1 and 2).
        # head_norm=False drops normalisation from the head entirely; the default
        # keeps a LayerNorm on the features before the projection.
        self.final_proj = FinalProjection(n_feat0, out_chans,
                                         dim_scale=patch_size,
                                         norm_output=False,
                                         norm_layer=nn.LayerNorm if kwargs.get('head_norm', True) else None)
...
class NormPromptUnet(nn.Module):
    ...
    def pad(self, x: torch.Tensor) -> Tuple[torch.Tensor, Tuple[List[int], List[int], int, int]]:
        ...
        # pad to a multiple of patch_size * 2**n_downblocks (3 DownBlocks)
        pad_to = self.unet.patch_size * (2 ** 3)
...
class PromptMR(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super().__init__()
        # upstream VSSD default; overridable from the config
        kwargs.setdefault('ssd_positve_dA', True)
