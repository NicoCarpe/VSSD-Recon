...
def build_radial_mask_bank(mat_folder, output_h5_path):
    ...
    3) Loads each .mat via load_mask():
         - If 2D, treat as shape (ny, nx) → expand to (1, ny, nx).
         - If 3D, assume shape is already (nt, ny, nx).
       Keep that as raw_3d = (nt, ny, nx).
    4) Keeps only the file with the largest nt for each unique (acc, ny, nx).
    5) Ensures the output folder exists, then writes each “best” mask into an HDF5
       under the key 'acc{acceleration}_{ny}x{nx}', storing data as (nt, ny, nx).
    """
    ...
    # Map (acc, height, width) → (best_filepath, best_nt)
    best_per_combo = {}
    ...
    for filepath in file_list:
        ...
        # If 2D, expand to (1, ny, nx); if 3D, assume (nt, ny, nx)
        if raw.ndim == 2:
            raw_3d = raw[np.newaxis, :, :]  # shape = (1, ny, nx)
        elif raw.ndim == 3:
            raw_3d = raw  # shape = (nt, ny, nx)
        else:
            print(f"  ▶ '{basename}' has invalid shape {raw.shape}; skipping.")
            continue

        nt, ny, nx = raw_3d.shape
        key = (acc, ny, nx)
    ...
    # 5) Write each “best” mask into the HDF5 as (nt, ny, nx)
    with h5py.File(output_h5_path, "w") as h5f:
        for (acc, ny, nx), (best_path, best_nt) in best_per_combo.items():
            basename = os.path.basename(best_path)

            # Reload and ensure shape (nt, ny, nx)
            try:
            ...
            if raw.ndim == 2:
                ...
                raw_3d = raw  # assume (nt, ny, nx)

            if raw_3d.shape != (best_nt, ny, nx):
                print(f"  Warning: '{basename}' shape {raw_3d.shape} != expected ({best_nt}, {ny}, {nx}); skipping.")
                continue

            dataset_name = f"acc{acc}_{ny}x{nx}"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "Gather all Radial-named .mat masks, assume 3D files come in as (nt, ny, nx), "
            "expand any 2D→(1, ny, nx), pick the largest nt per (acc, ny, nx), "
            "and save into one HDF5 as (nt, ny, nx)."
        )
    )
    parser.add_argument(
        "--mat_folder",
